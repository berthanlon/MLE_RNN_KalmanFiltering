...
class Plot_RTS(Plot_KF):
    ...
    def plotTraj_CA(self,test_target, RTS_out, rtsnet_out, dim, file_name):
        ...
        if dim==0:#position
            plt.plot(x_plt, rtsnet_out[0][0,:].detach().cpu().numpy(), label=legend[0])
            plt.plot(x_plt, test_target[0][0,:].detach().cpu().numpy(), label=legend[1])
            plt.plot(x_plt, RTS_out[0][0,:].cpu(), label=legend[2])    #BH
            ...
            plt.plot(x_plt, rtsnet_out[0][1,:].detach().cpu().numpy(), label=legend[0])
            plt.plot(x_plt, test_target[0][1,:].detach().cpu().numpy(), label=legend[1])
            plt.plot(x_plt, RTS_out[0][1,:].cpu(), label=legend[2])  #BH
            plt.legend(fontsize=font_size)
            plt.xlabel('t', fontsize=font_size)
            ...
            plt.plot(x_plt, rtsnet_out[0][2,:].detach().cpu().numpy(), label=legend[0])
            plt.plot(x_plt, test_target[0][2,:].detach().cpu().numpy(), label=legend[1])
            plt.plot(x_plt, RTS_out[0][2,:].cpu(), label=legend[2])  #BH
            plt.legend(fontsize=font_size)
            plt.xlabel('t', fontsize=font_size)
...
class Plot_extended(Plot_RTS):
    ...
    def plotTrajectories(self,inputs, dim, titles, file_name):
        ...
        i=0
        for title in titles:
            inputs_numpy = inputs[i][0].detach().cpu().numpy()
