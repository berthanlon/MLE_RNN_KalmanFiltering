        ...
        for ti in range(0, self.N_Epochs):
            ...
            for j in range(0, self.N_B):
                ...
                for t in range(0, self.ssModel.T):
                    print('y_training[:, t]', y_training[:, t])
                    print('self.model.__call__', self.model.__call__)
                    print('x_out_trainin shape', x_out_training.shape)
                    print('SELF.model(y_training[:, t]) shape', self.model(y_training[:, t]).shape)
                    x_out_training[:, t] = self.model(y_training[:, t])
                    #print('x_out_training[:, t]', x_out_training[:, t])
